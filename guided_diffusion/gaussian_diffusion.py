...
class GaussianDiffusion:
    def __init__(self,
        ...
        self.model_mean_type = model_mean_type  # 新增
        self.model_var_type = model_var_type    # 新增
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if model_output.shape[1] == 2 * x.shape[1]:
            ...
            model_var_values = model_output  # just use for variance calculation
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        if "betas" not in kwargs:
            raise ValueError("betas must be provided to SpacedDiffusion.")
        self.original_num_steps = len(kwargs["betas"])

        # Default arguments if not provided
        kwargs.setdefault("dynamic_threshold", False)
        kwargs.setdefault("clip_denoised", True)
        kwargs.setdefault("loss_type", LossType.MSE)
        kwargs.setdefault("model_mean_type", ModelMeanType.EPSILON)
        kwargs.setdefault("model_var_type", ModelVarType.FIXED_SMALL)
        kwargs.setdefault("rescale_timesteps", False)


        # Ensure correct Enum types
        if isinstance(kwargs["model_mean_type"], str):
            kwargs["model_mean_type"] = ModelMeanType[kwargs["model_mean_type"].upper()]
        if isinstance(kwargs["model_var_type"], str):
            kwargs["model_var_type"] = ModelVarType[kwargs["model_var_type"].upper()]
        if isinstance(kwargs["loss_type"], str):
            kwargs["loss_type"] = LossType[kwargs["loss_type"].upper()]

        # Construct base diffusion
        ...
        new_kwargs["betas"] = np.array(new_betas)

        # Init parent with new betas
    ...
    def _scale_timesteps(self, t):
        return t  # scaling is handled inside _WrappedModel
