...
class ddnm(ConditioningMethod):
    def __init__(self, operator, noiser, **kwargs):
        ...
        self.operator = operator
    def conditioning(self, x_0_hat, measurement, at, **kwargs):
        # x_0_hat -=  self.operator.transpose(self.operator.forward(x_0_hat)- measurement) * self.scale / at.sqrt()
        # norm = self.operator.forward(x_0_hat)- measurement
        # x_0_hat =  x_0_hat  - self.operator.transpose(norm) 

        norm_grad, norm = self.grad_and_value(x_prev=x_0_hat, x_0_hat=x_0_hat, measurement=measurement, **kwargs)
        # x_0_hat = x_0_hat.detach()
        x_0_hat = x_0_hat - norm_grad
...
class WoProjSampling(ConditioningMethod):
    ...
    def conditioning(self, x_0_hat, measurement, at, **kwargs):
        norm_grad, norm = self.grad_and_value(x_prev=x_0_hat, x_0_hat=x_0_hat, measurement=measurement, **kwargs)
        # x_0_hat = x_0_hat.detach()
        x_0_hat = x_0_hat - norm_grad * self.scale / at.sqrt()
        # x_0_hat = x_0_hat - 0.5* norm_grad
        return x_0_hat, norm
...
class AESampling(ConditioningMethod):
    ...
    def conditioning(self, x_0_hat, measurement, at, **kwargs):
        ...
        if 0.5 > t > 0.3:
            ...
            # x_0_hat = x_0_hat.detach()
            x_0_hat = x_0_hat - norm_grad * self.scale*0.0075 / at.sqrt()
        else:
            norm_grad, norm = self.grad_and_value(x_prev=x_0_hat, x_0_hat=x_0_hat, measurement=measurement, **kwargs)
            # x_0_hat = x_0_hat.detach()
            x_0_hat = x_0_hat -  norm_grad * self.scale / at.sqrt()
...
class LatentSampling(ConditioningMethod):
    ...
    def conditioning(self, x_0_hat, measurement, at, **kwargs):
        ...
        if 0.5 > t > 0.3:
            ...
            # E_x0_t = E_x0_t.detach()
            E_x0_t = E_x0_t - self.scale*0.01*norm_grad / at[0,0,0,0].sqrt()
            D_x0_t = self.ldm_model.decode_first_stage(E_x0_t)
            x_0_hat = D_x0_t + diff
            # x_0_hat = x_0_hat.detach()
        else:
